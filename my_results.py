...
rows = []
with open("tarry_1to40agents_250iterations_matlabdb_10x10.csv", "r") as f:
    csvreader = csv.reader(f)

    for row in csvreader:
        rows.append([float(e) for e in row])

tarry_matlab_steps_row = rows[1]
tarry_matlab_pionner_steps_row = rows[2]
tarry_matlab_fraction_row = rows[3]
tarry_matlab_stdev_row = rows[4]
tarry_matlab_steps_from_first_to_last_row = rows[5]
...
plt.plot(header, steps_row, label = "Average")
plt.plot(header, stdev_row, label = "STDEV")
plt.plot(header, steps_from_first_to_last_row, label = "Average Steps From Fist to Last")
plt.plot(header, fraction_row, label = "Fraction of maze explored")
 """
...
plt.title('Tarry My maze vs Tarry Matlab maze - 10-by-10 maze')
